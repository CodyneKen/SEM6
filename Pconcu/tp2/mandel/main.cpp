...
static mutex m_last_slice;
static mutex m_draw_rec;
...
int get_slice() {
    unique_lock<mutex> lock(m_last_slice); // mutex lock line
...
void draw_screen_worker(int& counter) {
    while (1) {
        int i = get_slice();
        if (i == -1)
            return;
...
int draw_screen_thread(int number_of_threads) {
    int sum = 0;
    int i = 0;
    vector<thread> tab;
    int tab_counter[number_of_threads] = {0};
    

    for(i=0; i<number_of_threads; i++){
        tab.push_back(thread(draw_screen_worker, ref(tab_counter[i])));
    }
    for(i=0; i<number_of_threads; i++){
        tab[i].join();
        sum+=tab_counter[i];
...
int main(int argc, char ** argv) {
    ...
    while(1) {
        ...
        if (use_threads) {
            int n = draw_screen_thread(number_of_threads);
