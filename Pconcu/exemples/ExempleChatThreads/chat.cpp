...
void producteur(const char *argument){
  ...
  while (1) {
    ...
    //########################################################
    // part 3
    // Décommenter pour mettre en place les mutex
    // m.lock();
    //# Fin ##################################################
    //########################################################
    // part 3.ter
    // Décommenter pour mettre en place les mutex à libération automatique
    ...
    if (nouveau_message) { 
      ...
      //########################################################
      // part 4
      // Commenter pour mettre en place les variables de condition
    ...
    nouveau_message = TRUE; // On signale la présence d'un nouveau message

    //########################################################
    // part 2
    // Décommenter pour tester la section critique
    usleep(50000);
    //# Fin ##################################################
...
gboolean consommateur(void *rien) {
  ...
  if (!(nouveau_message)) {
    ...
    nouveau_message = FALSE; // On signale que le message a été lu
...
main (int argc, char *argv[])
{
  ...
  if (argc == 2) {
    ...
    //########################################################
    // part 1
    // Décommenter pour lancer les threads
  ...
  g_timeout_add(5000, //nb de millisecondes entre les appel
